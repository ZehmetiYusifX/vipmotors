...
export function useChatWidget() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: "bot", text: "Bağlantı xətası. Yenidən cəhd edin." }]);
    } finally {
      setLoading(false);
    }
  };

  return { open, setOpen, messages, input, setInput, loading, send, image, setImage };
}

export default function ChatWidget() {
  const { open, setOpen, messages, input, setInput, loading, send, image, setImage } = useChatWidget();
        ...
        {open && (
          ...
          >
            ...
            <form
              ...
              className="border-t border-white/5 bg-ink-900/60 p-3 flex flex-col gap-2"
            >
              {image && (
